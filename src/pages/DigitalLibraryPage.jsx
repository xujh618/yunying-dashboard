...
const DigitalLibraryPage = () => {
  ...
  // 设置初始时间范围为最近6个月
  const initialStartMonth = format(subMonths(new Date(), 5), 'yyyyMM');
  const initialEndMonth = format(new Date(), 'yyyyMM');
  ...
  const fetchPlatformData = async () => {
    try {
      ...
      console.error('获取数字智库数据失败:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6">数字智库 - 运营数据</h1>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        <h2 className="text-xl font-bold mb-4">数字智库 - 运营数据趋势</h2>
        <PlatformChart
          chartType="line"
